...
var app = angular.module('bbb', ['btford.socket-io', 'highcharts-ng']);

app.controller('bbbController', ['$scope', '$interval', 'socket', function ($scope, $interval, socket) {
    $scope.showRollsGraph = false;
    $scope.spentTime = 0
    $scope.max = {
        betAmount: 0,
        payout: 0,
    ...
    mySocket.on('roll', function (data) {
        ...
        if (data.data.roll.win) $scope.wins++;
        else $scope.loses++;
        //maxs
        /*
